...
for kn in range(1,12):
    ...
    ppp6 = []
    ypp = []
#    directory = 'sp17kn/sp'+str(kn)
    directory = 'ch17kn/ch'+str(kn)
    print directory
    pk = []
    kk = 0
    for k in range(1,301):
          ...
          yp = abs((abs(ppi1)-abs(ppi2)))/(abs(ppi1)+abs(ppi2))
          N=len(xi)
          fs=1/(xi[2]-xi[1])
          n=arange(N)
          f=n*fs/N
...
#该模块为求位相，力，加速度，有效质量的实数部分和虚数部分
          ...
          b1=abs(b1)
          print k,b1/(2*pi),a1,a2,m1,m2,rsquare1,rsquare2,wxc,power,ppi
          if rsquare1>0:
             pf.append(b1/(2*pi))
             pa1.append(a1)
             pa2.append(a2)
    ...
    plt.ylabel(u'm')
    plt.plot(pf,pm1,'r*-',label='m1')
    plt.plot(pf,pm2,'bo-',label='m2')
    ...
    plot3 = plt.plot(pf,pa2,'bo')

    plt.sca(ax4)
    plt.title('Goodness of fit')
    plt.xlabel(u'frequency')
    plt.ylabel(u'rsquare')
    plt.plot(pf,pr1,'r*-',label='rsquare1')
    plt.plot(pf,pr2,'bo-',label='rsquare2')
    ...
    yl.append(mean(ypp))
    yq.append(mean(ppp))
fyq = open(str(txt3.lj)+'data2/chyl.txt','w')
for zn in range(len(yq)):      
    print >>fyq,yq[zn],yl[zn]
fyq.close
plt.figure(2)
plt.plot(yq,yl,'bo-')
savefig(str(txt3.lj)+'data2/chyl.png')
plt.show()
adf
